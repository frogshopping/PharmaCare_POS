...
import { MedicineType, getMedicineTypes } from '@/services/api';
import { Plus, Eye, Edit, Trash2 } from 'lucide-react';

export default function MedicineTypePage() {
    const [medicineTypes, setMedicineTypes] = useState<MedicineType[]>([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        getMedicineTypes().then((data) => {
        ...
        <DashboardLayout>
            <div className="min-h-screen">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-2xl font-light text-slate-700">Medicine_type</h1>
                                ...
                                <tbody>
                                    {medicineTypes.map((item) => (
                                        <tr key={item.id} className="border-b hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 text-blue-600">{item.title}</td>
